#Subsampling scheme for AWI229-11_sv1057
#Originally written by Danielle Harris, 13 Apr 2017
#Parameters adjusted for site by Brian Miller, 22 Jun 2018

#Assuming 1 year of data, in hourly files and we want to analyse ~200 hours of data

site.code<-"awi229-11_sv1057"
#######################################################################################
...
start.date.time<-ISOdatetime(2015, 01, 01, 0, 0, 0, tz="GMT")
end.date.time<-ISOdatetime(2015, 12, 31, 23, 0, 0, tz="GMT") 
total.days<-as.numeric(end.date.time - start.date.time)
...
#so this sequence starts with the random start hour on the first day, then increases using the spacing required as a step length, and generates the required number of samples.  Finally, the whole sequence is rounded to the nearest integer hour.
...
plot(extract.days,extract.months)
plot(extract.months,extract.hours)

write.csv(data.frame(HourNumber=1:length(random.sample), Name=site.code,
                     StartTime=random.sample, StopTime=random.sample+3600), 
          file=paste(site.code,"_subsample.csv"), row.names = FALSE)
