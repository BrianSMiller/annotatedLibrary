...
% Generate a subsample of hours from an acoustic recording.
% Sample data using a fixed sample rate with a random starting time.
% fileInfo contains the metadata from an acoustic recording as a
% soundFolder data structure (see soundFolder.m). 
% Params is a data structure with the following fields
%  params.code: string with a unique site/recording code that will be
%   used when creating output files and folders. e.g. params.code =  'casey2014';
%     
%  params.inputFolder: path that matches the location of your sound
%   files. e.g  params.inputFolder = 'M:\Casey2014\';
%          
%  params.outpuFolder: Output files will go into this location
%   e.g. params.outputFolder = '\\aad.gov.au\files\ftproot\Public\BrianMiller\sorp\test\'; 
%    
%  params.numberOfChunks: Total number of discrete subsampling periods (i.e. chunks).
%   e.g. params.numberOfChunks = 200;
%     
%  params.durationOfChunk: The duration of each chunk in seconds
%   params.durationOfChunk = 3600;
%     
%  params.outputSampleRate: Resample wav files to this rate (Hz) if
%   creating output files. e.g. params.outputSampleRate = 1000;
%     
%  params.refreshFileInfo: If refreshFileInfo is true, then the wav
%   metadata and timestamps will always be loaded from disk. Otherwise, the
%   wav metadata and timestamps will be cached so that they load quickly.
%   params.refreshFileInfo = false;
%     
%  params.createOutputWavFiles: If createOutputWavFiles is true, new wav
%   files will be created, otherwise the filenames, start and stop times
%   are printed to the console params.e.g. createOutputWavFiles = true;
%     
%  params.constantSubSampleRate: If constantRate is true, then the
%   subsample rate will be the number of numberOfChunks/(365*24). This may
%   result in fewer than numberOfChunks sub samples if the duration of
%   recording is less than 1 full year. If constantRate is false, the total
%   number of chunks will be evenly distributed throughout the full
%   duration of the recording. e.g. params.constantSubSampleRate = false;
